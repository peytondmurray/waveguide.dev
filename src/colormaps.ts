 ...
 */
function rgbToLin(val: number): number {
 ...
 *
 * @param rgb - RGB pixel to convert
 * @returns The 'linearized' value associated with the RGB pixel
 */
...
/**
 * Convert a flat RGB array to a relative luminance array.
 *
 * @param rgb - Flat RGB array to convert. Values are RGB tuples for each pixel:
 *  [R_0, G_0, B_0, R_1, G_1, B_1, ..., R_N, G_N, B_N]
 * @returns A flat array of relative luminance, i.e.
 *  [Y_0, Y_1, ..., Y_N]
 */
 ...
 *
 * @param cmap - RGB array to use as a colormap. Lower values are mapped to lower indices
 * @param value - Value for which the nearest colormap entry should be retrieved; must exist on the
 * range [0, 1]
 */
...
/**
 * Convert a flat RGB pixel to an RGBA value generated by a colormap.
 *
 * Linear interpolation between the colormap value just below and just above the value of the pixel
 * is used.
 *
...
export function toCmap(
  ...
  // Associate the lowest index of the colormap with the lowest value,
  // and the highest index with the highest value.
